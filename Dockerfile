...
ENV MYSQL_HOST 10.0.224.7:3306
ENV MYSQL_USER_NAME root
ENV MYSQL_PASSWORD 12345Qaz
ENV DATABASE_NAME music-l
...
# 将构建产物jar包拷贝到运行时目录中
COPY --from=build /app/target/music-l-0.0.1.jar .

# 暴露端口
EXPOSE 80

# 执行启动命令
CMD ["java", "-jar", "/app/music-l-0.0.1.jar", "--spring.profiles.active=test"]